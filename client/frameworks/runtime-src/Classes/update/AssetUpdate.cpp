...
void AssetUpdate::startUpdate()
{
	...
	_actionTimeOut = Sequence::create(DelayTime::create(TIME_OUT),
		CallFunc::create([this](){
				...
				if (this->_updateRetry >= 2){
					onUpdateError(Localization::text("TXT_CDN_UPDATE_FAILED"), ERROR_DOWLOAD_VERSION_MANIFEST_TIMEOUT);
				}
				else
				{
					this->startUpdate();
	...
	if (!_am->getLocalManifest()->isLoaded())
	{
		...
		onUpdateError(Localization::text("TXT_CDN_NO_MANIFEST"), ERROR_NO_LOCAL_MANIFEST);
	}
	else
	{
		...
		_amListener = cocos2d::extension::EventListenerAssetsManagerEx::create(_am, [this](EventAssetsManagerEx* event){
			...
			case EventAssetsManagerEx::EventCode::ERROR_NO_LOCAL_MANIFEST:
			{
				...
				onUpdateError(Localization::text("TXT_CDN_NO_MANIFEST"), (int)event->getEventCode());
			}
			break;
			...
			{
				onUpdateError(Localization::text("TXT_CDN_DOWNLOAD_MANIFEST_FAILED"), (int)event->getEventCode());
			}
			break;
			case EventAssetsManagerEx::EventCode::ERROR_PARSE_MANIFEST:
			{
				
				onUpdateError(Localization::text("TXT_CDN_PARSE_MANIFEST_FAILED"), (int)event->getEventCode());
			}
			break;
			...
			case EventAssetsManagerEx::EventCode::UPDATE_FAILED:
			{
				...
				else
				{
					...
					failCount = 0;
					onUpdateError(Localization::text("TXT_CDN_UPDATE_FAILED"), (int)event->getEventCode());
				}
			}
			break;
