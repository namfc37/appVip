...
void ScreenUpdateAssets::initSplash()
{
	...
	auto bg = Sprite::create("res/home/home_bg.png");
	bg->setPosition(size.width / 2, size.height / 2);
	scaleSpriteToFull(bg, size);
	this->addChild(bg, 0);

	auto title = Sprite::create("res/home/home_logo.png");
	title->setPosition(size.width / 2, size.height / 2);
	title->setVisible(false);
	this->addChild(title, 10);

	//Size size = Director::getInstance()->getVisibleSize();
	_loadingText = Text::create("Loading...", "res/common/ui/iCielPanton-Black.ttf", 32);
...
void ScreenUpdateAssets::showAssetsLoading()
{
	if (_state != LoadingState::UPDATING){
		...
		if (_nodeAssetLoading){
			this->addChild(_nodeAssetLoading, 20);
			Size size = Director::getInstance()->getVisibleSize();
			_nodeAssetLoading->setContentSize(size);

			_textStatus = Text::create("", "res/common/ui/iCielPanton-Black.ttf", 32);
			if (_textStatus)
			{
				_textStatus->setString("");
			}
			_textStatus->setPosition(Vec2(size.width / 2, 20));
...
void ScreenUpdateAssets::onUpdateError(const std::string& err, int errCode)
{
	...
	if (dialog){
		...
		Text* errText = Text::create(msg, "res/common/ui/iCielPanton-Black.ttf", 20);
		errText->setAnchorPoint(Vec2(1, 0));
		errText->setPosition(Vec2(winSize.width - 10, 10));
		dialog->addChild(errText);

		addChild(dialog, 30);
		dialog->showOneButton(err, Localization::text("TXT_CDN_RETRY"), //dialog->showOneButton(err, Localization::text("lang_btn_try_again"),
