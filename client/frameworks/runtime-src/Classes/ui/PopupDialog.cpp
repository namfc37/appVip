...
bool PopupDialog::init()
{
	...
	_oneButtonNode->setPosition(bgW / 2, 50); //_oneButtonNode->setPosition(bgW / 2, bgH * 0.31f);
	//two buttons
	 _twoButtonNode = Node::create();
	 bg->addChild(_twoButtonNode);
	 _twoButtonNode->setPosition(bgW / 2, bgH * 0.31f);

	 _tileText = Text::create("Text", "res/common/ui/iCielPanton-Black.ttf", 28 / scaleFactor);
	 _tileText->setColor(Color3B(115, 115, 115));
	 _tileText->setPosition(Vec2(bgW / 2, 100)); //_tileText->setPosition(Vec2(bgW / 2, bgH * 0.82f));
...
{
	float scaleFactor = Director::getInstance()->getContentScaleFactor();
	_one_button = Button::create("res/home/home_button.png");
	_one_button->setPressedActionEnabled(true);
	_one_button->setTitleFontName("res/common/ui/iCielPanton-Black.ttf");
	_one_button->setTitleColor(Color3B(252, 252, 252));
	_one_button->setTitleFontSize(22); //_one_button->setTitleFontSize(getFontButtonSize() / scaleFactor);
	_one_button->setContentSize(Size(228, 54));// locallize _one_button->setContentSize(Size(150, 50));
	_one_button->setScale9Enabled(true);
