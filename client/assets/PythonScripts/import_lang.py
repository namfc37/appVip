...
def import_to_lang(dataFolder, out_file):

    list_data = {}
    list_key = []
    list_file = []
    for file in glob.glob(dataFolder + "/*.txt"):
        list_file.append(file)
    for i in range(0, len(list_file), 1):
        with open(list_file[i]) as f:
            content = f.read().decode("utf-8-sig").encode("utf-8").splitlines()
            for line in content:
...
dataFolder = "./client/assets/LanguagesDataImport"
import_to_lang(dataFolder, './client/assets/LanguagesData/lang.csv')