...
public class Event01 extends ParseWorkbook
{
	public Event01Info info;
	
	public Event01 (String inputName) throws Exception
	{
		super(inputName);
	}
...
//		parseShop("Shop");
	...
	{
		this.info = 					new Event01Info();
		this.info.E01_TYPE =			Define.miscJson.get("EV01_TYPE").getAsString();
		this.info.E01_ID =				Define.miscJson.get("EV01_ID").getAsString();
		this.info.E01_POINT =			Define.miscJson.get("EV01_POINT").getAsString();
		this.info.E01_PLANT =			Define.miscJson.get("EV01_PLANT").getAsString();
		this.info.E01_DROPITEM =		Define.miscJson.get("EV01_DROP_ITEM").getAsString();
		this.info.E01_START_TIME =		Define.miscJson.get("EV01_TIME_START").getAsString();
		this.info.E01_END_TIME =		Define.miscJson.get("EV01_TIME_END").getAsString();

//		convert to itemId
		this.info.E01_ID =				Util.toItemId (this.info.E01_ID);
		this.info.E01_POINT =			Util.toItemId (this.info.E01_POINT);
		this.info.E01_PLANT =			Util.toItemId (this.info.E01_PLANT);
		this.info.E01_DROPITEM =		Util.toItemId (this.info.E01_DROPITEM);
		
//		override key
		Define.miscJson.addProperty("EV01_ID", this.info.E01_ID);
		Define.miscJson.addProperty("EV01_POINT", this.info.E01_POINT);
		Define.miscJson.addProperty("EV01_PLANT", this.info.E01_PLANT);
		Define.miscJson.addProperty("EV01_DROP_ITEM", this.info.E01_DROPITEM);
		
//		convert time
		this.info.E01_UNIX_START_TIME =	Util.toUnixTime(this.info.E01_START_TIME);
		this.info.E01_UNIX_END_TIME =	Util.toUnixTime(this.info.E01_END_TIME);
		EventInfo.setEventDuration(this.info.E01_ID, this.info.E01_UNIX_START_TIME, this.info.E01_UNIX_END_TIME);
		
		this.info.E01_PLANT_DROP_LIST = new PlantDropItemInfo ();
		this.info.E01_FEATURE_DROP_LIST = new FeatureDropItemInfo (this.info.E01_DROPITEM);
		this.info.E01_PUZZLE = new PuzzleInfo();
		
		this.info.E01_SHOP = new IBShopInfo();
		this.info.E01_SHOP.TAB = "event";
		this.info.E01_SHOP.NAME = "event";
		this.info.E01_SHOP.GFX = "";
		this.info.E01_SHOP.ITEMS = new ArrayList<Item> ();

		this.info.E01_REWARDS = new TreeMap<Integer, TreeMap<Integer, Event01Reward>> ();
	}
	...
	private void parseFeatureDrop (String idSheet)
	{
		...
		for (int r = 1; r <= maxRow; r++)
		{
			...
			this.info.E01_FEATURE_DROP_LIST.add(featureActionId, option, rate, quantity, dailyLimit);
		}
	}
	...
	private void parsePlantDrop (String idSheet)
	{
		...
		for (int r = 1; r <= maxRow; r++)
		{
			...
			this.info.E01_PLANT_DROP_LIST.add(item, rate, min, max, userLimit, serverLimit, isVietNamOnly);
		}
	}
	
	private void parseRewards (String idSheet)
	{
		HashMap<Integer, Event01Reward> checkIds = new HashMap<Integer, Event01Reward> ();
		...
		for (int r = 1; r <= maxRow; r++)
		{
			...
			Event01Reward reward = new Event01Reward (total);
			reward.id = id;
			...
			if (reward.check())
			{
				TreeMap<Integer, Event01Reward> checkpoint_rewards = this.info.E01_REWARDS.get(checkpoint);
				if (checkpoint_rewards == null)
				{
					checkpoint_rewards = new TreeMap<Integer, Event01Reward> ();
					this.info.E01_REWARDS.put(checkpoint, checkpoint_rewards);
				}
	...
	private void parsePuzzle (String idSheet)
	{
		...
		for (int r = 1; r <= maxRow; r++)
		{
			...
			this.info.E01_PUZZLE.add(id, require, rewards, displayOrder, isVietNamOnly);
		}
	}
}