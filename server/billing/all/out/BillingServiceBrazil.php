...
$objCard->net = $data['NetAmount'] * 100; //dollar to cent
$objCard->gross = $data['GrossAmount'] * 100; //dollar to cent
...
$objCard->coinConvert = intval($embedData['gross'] / RATE_VND_TO_COIN);
$objCard->convertCash = intval($embedData['net'] / RATE_VND_TO_COIN);
...
function validateMac ($data, $keyHash)
{
    ...
    ksort($array);	
    $check = md5(implode($array).$CONFIG_DATA['BRAZIL_SECRET']);
    return $hash == $check;
}
