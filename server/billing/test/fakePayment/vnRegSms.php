...
else
{
    ...
    else
    {
        ...
        $data->channel = 5; //1: thẻ zing, 2: thẻ mobile, 3: thẻ vina, 4: thẻ viettel, 5: sms, 6 :atm, 36 :credit card, 38: zalopay.
        $data->amount = $gross;
        ...
        {            
            ...
            {
                $sms = new stdClass();    
                $sms->servicePhone = '900';
                $sms->smsSyntax = 'test atm reg';

                $res->returnCode = 1;
                $res->returnMessage = '';                
                $res->smsServicePhones = $sms;
            }
            else 
            {
                $res->returnCode = -5;
